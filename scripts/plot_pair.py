...
# 2 inputs: base currency and term currency
# run >>> python plot_pair.py AUD USD
...
term_currency = sys.argv[3]


# Buterworth filter fast
N = 5  # Filter order
Wn = 0.08  # Cutoff frequency
...
base = pd.read_csv('db/indexes/'+timeframe+'/'+base_currency+'.csv')
base[base_currency] = round(base[base_currency].cumsum(), 2)
# lowpass filter fast
base[base_currency+'f'] = signal.filtfilt(Bf, Af, base[base_currency], axis=0)
# lowpass filter slow
#base[base_currency+'s'] = signal.filtfilt(Bs, As, base[base_currency])

# Apply the filter to term currency
term = pd.read_csv('db/indexes/'+timeframe+'/'+term_currency+'.csv')
term[term_currency] = round(term[term_currency].cumsum(), 2)
# lowpass filter fast
term[term_currency+'f'] = signal.filtfilt(Bf, Af, term[term_currency], axis=0)
# lowpass filter slow
